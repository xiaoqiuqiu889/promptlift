...
- **桌面精灵可缩放**：鼠标移到桌面精灵右上角的三个轻量圆点即可拖动改变大小；小狗、圆点和状态条会作为一个整体等比响应，并限制在安全尺寸内，避免异常裁切和大块留白。
...
当前版本的自动化回归为 **232/232 通过**，提示词边界循环为 **24/24 通过**；真实模型契约确认模型不会擅自追加继续开发的请示句，新增回归同时阻止“待审计”被改成“已完成审计”以及清楚任务被追加非阻塞确认问题。Windows 界面验收覆盖 **85 个状态、204 次可信点击和 216 次可信手势**，包含 16 组场景 × 档位提示词切换、单滚动提示词页、双页签导航、语义图标、小狗待机动画、紧凑视觉簇缩放以及默认双击 Alt 等关键路径；几何缺陷与流程缺陷均为 0。
