...
function assertDirectRewriteResult(result, source, language, mode) {
  ...
  const metaRewriteFrame = /(?:请|需要|任务是).{0,12}(?:将|把)?(?:以下|下列|这段|上述).{0,32}(?:优化|润色|改写|重写|增强)|(?:用户反馈原文|消息原文|待(?:优化|润色|改写)(?:文本|内容)|(?:优化|润色|改写)要求)\s*[:：]/isu;
  const englishMetaRewriteFrame = /(?:please|task is to).{0,20}(?:optimi[sz]e|polish|rewrite|improve).{0,24}(?:the following|source|original|user feedback)|(?:source|original|user feedback)\s*:/isu;
...
export function buildModelInstruction(
  ...
  const styleInstructions = language === 'zh'
    ? {
      faithful: '严格保真：只做必要澄清和组织，不扩大范围，不添加原文没有依据的要求。',
      balanced: '保持清晰、完整和实用。',
      concise: '优先使用简洁表达，删除不必要的重复。',
      detailed: '在原意允许的范围内补充有用的具体细节和执行信息。',
      professional: '使用正式、结构化、专业的表达。',
      creative: '使用生动但准确的表达，同时保持可执行。',
    }
    : {
      faithful: 'Strictly preserve meaning: only clarify and organize what is necessary; do not expand scope or add unsupported requirements.',
      balanced: 'Keep the result clear, complete, and practical.',
      concise: 'Prefer concise wording and remove unnecessary repetition.',
      detailed: 'Add useful specificity and implementation detail where the original intent supports it.',
      professional: 'Use a formal, structured, and professional tone.',
      creative: 'Use vivid but precise wording while keeping the request actionable.',
    };
  const selectedStyle = Object.hasOwn(styleInstructions, style) ? style : MODEL_STYLES.balanced;
  if (language === 'zh') {
    ...
    return [
      ...
      `风格要求：${styleInstructions[selectedStyle]}`,
  ...
  return [
    ...
    `Style requirement: ${styleInstructions[selectedStyle]}`,
    recipe.id === RECIPE_IDS.chatPolish
...
export function buildModelMessages(prompt, language, options = {}) {
  ...
  const repairInstruction = options.repairMetaPrompt === true
    ? language === 'zh'
      ? '\n上一次结果错误地返回了二次改写任务或系统约束。请直接输出优化后的用户请求，不要解释如何改写，不要复述原文、规则或协议。'
      : '\nThe previous result incorrectly returned a meta-rewrite task or system constraints. Return the optimized user request itself; do not explain the rewrite or repeat the source, rules, or protocol.'
    : '';
...
async function enhanceWithOpenAICompatible(prompt, options, language) {
    ...
    try {
      response = await fetchImpl(endpoint, {
        ...
        body: JSON.stringify({
          model,
          stream: false,
          temperature: options.probe === true ? 0 : 0.2,
  ...
  const operation = (async () => {
    try {
      ...
      if (options.probe === true || error?.code !== 'MODEL_OUTPUT_META_PROMPT') {
        throw error;
      }
