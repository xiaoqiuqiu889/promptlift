...
- **场景与档位同页联动**：四种表达场景分别使用自己的提示策略和档位定义；场景选择已合并到首个“处理”页，不再占用独立页签。
...
4. 在首个“处理”页直接选择 AI 提示词、向上沟通、用户沟通或 PPT 文案，并选择对应优化档位；底部导航只保留“处理 / 服务 / 我的”。
...
当前版本的自动化回归为 **210/210 通过**，提示词边界循环为 **22/22 通过**；真实模型契约确认模型不会擅自追加继续开发的请示句。Windows 界面验收覆盖 85 个状态，包含默认双击 Alt、自定义快捷键录制与恢复、取消、审阅、恢复、处理页内场景切换、档位差异、系统提示词查看/编辑、三种小精灵和拖拽命中测试。
