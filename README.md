...
- **桌面精灵可缩放**：鼠标移到桌面精灵右上角即可拖动改变大小；缩放会保持当前宽高比，并限制在安全尺寸内，避免角色被异常裁切。
...
当前版本的自动化回归为 **223/223 通过**，提示词边界循环为 **22/22 通过**；真实模型契约确认模型不会擅自追加继续开发的请示句。Windows 界面验收覆盖 **85 个状态、204 次可信点击和 216 次可信手势**，包含 16 组场景 × 档位提示词切换、单滚动提示词页、双页签导航、小狗待机动画、右上角等比缩放以及默认双击 Alt 等关键路径；几何缺陷与流程缺陷均为 0。
